...
static void* vaddr_get(enum pool_flags flag, uint32_t pg_cnt) {
    int index;
    if (flag == PF_USER) {
        // TODO
        return NULL;
    ...
    return index == -1 ? NULL : kernel_vaddr_pool.vaddr_start + index * PG_SIZE;
}
...
static void* palloc(struct paddr_pool* m_pool) {
    ...
    return index == -1 ? NULL : m_pool->phy_addr_start + index * PG_SIZE;
}
